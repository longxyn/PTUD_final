...
$data1 =array('nhacungcap'=> $list1);
        ...
        foreach ($reg2->fetchAll() as $item){
            $list2[] =new Sanpham($item['Id'],$item['TenSP'],$item['IdDVT'],$item['IdNCC'],$item['SoLuong'],$item['TrangThai'],$item['id_kho_sp']);
                        }
        $data2 =array('sanpham'=> $list2);
...
if(isset($_POST['create-sp'])){
    ...
    $ksp = $_POST["ksp"];
    SanPham::update($id,$ten,$dvt,$ncc,$soluong,$TrangThai,$ksp);
}
?>
