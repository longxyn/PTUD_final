...
<!--   end //-->
    ...
    <div class="form-row ml-4">
               ...
               <optgroup label="chọn sản phẩm">
                   ...
                   foreach ($list2 as $item){
                    if($item->SoLuong > 0){
                        ...
                        echo "<option value='$item->Id' data-sp_sl='$item->SoLuong' data-sp_gia='$item->GiaBan' >" . $item->TenSP . " (Số lượng: " . $item->SoLuong . ")</option>";
        ...
        <div class="col-md-3 form-group mb-3">
            <label for="validationDefault01">Số lượng xuất kho</label>
...
//    echo  number_format($tongdon,0,".",",");
    ...
    DonBan::add($ngayban,$nhanvien,$khachhang,$trangthai);

    $donban = [];
    $db_db =DB::getInstance();
    $reg_db = $db_db->query('SELECT * FROM DonBan ORDER BY Id DESC');
    foreach ($reg_db->fetchAll() as $item){
        $donban[] =new DonBan($item['Id'],$item['NgayBan'],$item['IdNV'],$item['IdKH'],$item['TrangThai']);;
    }
    echo $donban[0]->Id;
    $IdDon = $donban[0]->Id;
    echo $khachhang;
    ...
    for($i = 0; $i < count($arr_sp_ma); $i++) {
        ...
        $soluongcu = $arr_sp[$i];
    ChiTietBan::add($IdDon,$sp_ma,$sp_dh_soluong,0);
